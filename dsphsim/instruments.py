...
class IMACS(Instrument):
    ...
    def snr2err(cls, snr):
        ...
        a,b = -1.2, 1.5
        a,b=-1,1
        # From Heiger 2024, this is a good fit
        return 10**(a*np.log10(snr) + b)
...
class GCLEF(Instrument):
    """https://gclef.cfa.harvard.edu/etc/"""

    _filename = 'gclef_g.dat'
    _exptime0 = 7200.
 
    #copied from M2FS one
    _defaults = odict([
        ('vsys',  0.01),
        ('fov',   706.9),
        ('nstar', 256),
    ...
    @classmethod
    def snr2err(cls, snr):
        """based off the DEIMOS one, but we assume based on AP Hatzes 2019
           that v_err ~ SNR^-1 R^-1. Then the difference in b comes from difference in resolution between DEIMOS and GCLEF. 1.5+log(6000/35000)=0.73"""
        a,b = -1.2, 0.73
        return 10**(a*np.log10(snr) + b)

class VIASPEC(Instrument):
    """https://gclef.cfa.harvard.edu/etc/"""
